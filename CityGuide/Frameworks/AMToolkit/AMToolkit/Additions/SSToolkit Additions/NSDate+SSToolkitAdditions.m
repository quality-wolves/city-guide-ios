...
+ (NSString *)timeInWordsFromTimeInterval:(NSTimeInterval)intervalInSeconds includingSeconds:(BOOL)includeSeconds {
	...
	if (intervalInMinutes >= 0 && intervalInMinutes <= 1) {
		...
		if (intervalInSeconds >= 0 && intervalInSeconds < 5) {
			return [NSString stringWithFormat:NSLocalizedString(@"less than %d seconds", @""), 5];
		} else if (intervalInSeconds >= 5 && intervalInSeconds < 10) {
			return [NSString stringWithFormat:NSLocalizedString(@"less than %d seconds", @""), 10];
		...
		return [NSString stringWithFormat:NSLocalizedString(@"%d minutes", @""), (NSInteger)intervalInMinutes];
		...
		return [NSString stringWithFormat:NSLocalizedString(@"about %d hours", @""), (NSInteger)round(intervalInMinutes / 60.0f)];
		...
		return [NSString stringWithFormat:NSLocalizedString(@"%d days", @""), (NSInteger)round(intervalInMinutes / 1440.0f)];
		...
		return [NSString stringWithFormat:NSLocalizedString(@"%d months", @""), (NSInteger)round(intervalInMinutes / 43200.0f)];
	} else if (intervalInMinutes >= 525600 && intervalInMinutes <= 1051199) {
		return NSLocalizedString(@"about 1 year", @"");
	} else {
		return [NSString stringWithFormat:NSLocalizedString(@"over %d years", @""), (NSInteger)round(intervalInMinutes / 525600.0f)];
	}
	return nil;
}
