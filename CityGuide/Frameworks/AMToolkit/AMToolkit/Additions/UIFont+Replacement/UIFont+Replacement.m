...
+ (void) load
{
	...
	Method fontWithName_size_traits_ = class_getClassMethod([UIFont class], (SEL)NSSelectorFromString(@"fontWithName:size:traits:")); //NSSelectorFromString to prevent Application Loader to warn about non-public selector usage
...
+ (UIFont *) replacement_fontWithDescriptor:(UIFontDescriptor*)descriptor size:(CGFloat)fontSize{
	...
	if (replacementFontName) {
		NSMutableDictionary *attribbutes = [descriptor.fontAttributes mutableCopy];
		[attribbutes setObject:replacementFontName forKey:UIFontDescriptorNameAttribute];
		descriptor = [UIFontDescriptor fontDescriptorWithFontAttributes:attribbutes];
	}
	return [self replacement_fontWithDescriptor:descriptor size:fontSize];
}
