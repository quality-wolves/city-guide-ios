...
@property (nonatomic) MSRefreshableDirections refreshableDirections;
@property (nonatomic) MSRefreshingDirections refreshingDirections;

// delegate to receive callbacks on different stages of the refresh cycle
@property (nonatomic, weak) id <MSPullToRefreshDelegate> delegate;
...
- (void) startRefreshingDirection:(MSRefreshDirection)direction animated:(BOOL)animated {
    ...
    switch (direction) {
        ...
        case MSRefreshDirectionBottom:
			...
			CGFloat correction = _scrollView.frame.size.height > _scrollView.contentSize.height ? _scrollView.contentSize.height - _scrollView.frame.size.height :0.0f;
            contentInset = UIEdgeInsetsMake(contentInset.top, contentInset.left, refreshingInset + correction, contentInset.right);
...
- (void) _checkOffsetsForDirection:(MSRefreshDirection)direction change:(NSDictionary *)change {
    ...
    switch (direction) {
        ...
        case MSRefreshDirectionBottom:
			...
			correction = _scrollView.frame.size.height > _scrollView.contentSize.height ? _scrollView.frame.size.height - _scrollView.contentSize.height :0.0f;
        ...
        case MSRefreshDirectionRight:
			...
			correction = _scrollView.frame.size.width > _scrollView.contentSize.width ? _scrollView.frame.size.width - _scrollView.contentSize.width :0.0f;
			
            canEngage = oldOffset.x + _scrollView.frame.size.width - adjustedContentSize.width > refreshableInset;
            contentInset = UIEdgeInsetsMake(contentInset.top, contentInset.left, contentInset.bottom, refreshingInset + correction);
            break;
        default:
            break;
    }
