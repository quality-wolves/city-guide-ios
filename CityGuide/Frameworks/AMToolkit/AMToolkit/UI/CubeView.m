...
#define kPerspectiveK -1550
#define kFadingColor [UIColor colorWithWhite:0.1f alpha:1.0f]

@interface CubeView () <UIScrollViewDelegate> {
    __strong UIScrollView *scrollView;
    __strong UIView *frontPane;
    __strong UIView *bottomPane;
    __strong UIView *topPane;
    __strong UIView *topEdgePane;
    __strong UIView *bottomEdgePane;
	
    __strong UIView *frontPaneShade;
    __strong UIView *bottomPaneShade;
    __strong UIView *topPaneShade;
    __strong UIView *topEdgePaneShade;
    __strong UIView *bottomEdgePaneShade;
	
    NSUInteger currentPage;
    CubeOrientation orientation;
	
    __weak id<CubeViewDelegate> delegate;
...
- (void)performInitialLayout {
    ...
    frontPane = [delegate viewForPage:0 CubeView:self];
    if ([delegate numPagesForCubeView:self] > 1) {
        bottomPane = [delegate viewForPage:1 CubeView:self];
    }
...
- (void)reload {
    ...
    frontPane = [delegate viewForPage:currentPage CubeView:self];
    if (currentPage > 0) {
        topPane = [delegate viewForPage:currentPage-1 CubeView:self];
    }
    if (currentPage < [delegate numPagesForCubeView:self]-1) {
        bottomPane = [delegate viewForPage:currentPage+1 CubeView:self];
    }
...
- (void)layoutPanes {
    ...
    if (topPane) {
        ...
        if (!topPaneShade || !topPaneShade.superview) {
            ...
            topPaneShade.backgroundColor = kFadingColor;
    ...
    if (bottomPane) {
        ...
        if (!bottomPaneShade || !bottomPaneShade.superview) {
            ...
            bottomPaneShade.backgroundColor = kFadingColor;
    ...
    if (!frontPaneShade || !frontPaneShade.superview) {
        ...
        frontPaneShade.backgroundColor = kFadingColor;
    }

    if (currentPage == 0 && !topEdgePane && [delegate respondsToSelector:@selector(topEdgePaneForCubeView:)]) {
        topEdgePane = [delegate topEdgePaneForCubeView:self];
        if (orientation == CubeOrientationHorizontal) {
        ...
        bottomEdgePane = [delegate bottomEdgePaneForCubeView:self];
        if (orientation == CubeOrientationHorizontal) {
...
- (void)scrollViewDidScroll:(UIScrollView *)sv {
    if (orientation == CubeOrientationHorizontal) {
        ...
        // Top panel rotation
        if (topPane != nil && (sv.contentOffset.x - frontPaneOffset) < 0) {
            ...
            xRotationTransform.m34 = 1.0 / kPerspectiveK;
        ...
        if (bottomPane != nil && (sv.contentOffset.x - frontPaneOffset) > 0) {
        ...
        // Top panel rotation
        if (topPane != nil && (sv.contentOffset.y - frontPaneOffset) < 0) {
			...
			yRotationTransform.m34 = 1.0 / kPerspectiveK;
        ...
        // Bottom panel rotation
        if (bottomPane != nil && (sv.contentOffset.y - frontPaneOffset) > 0) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if ([delegate respondsToSelector:@selector(CubeView:showedPageAtIndex:)]) {
        [delegate CubeView:self showedPageAtIndex:currentPage];
    }
}

- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    if (!CGRectContainsPoint([delegate edgeRectForPage:currentPage CubeView:self], point)) {
