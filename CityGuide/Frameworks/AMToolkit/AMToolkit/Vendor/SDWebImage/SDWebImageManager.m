...
- (id <SDWebImageOperation>)downloadWithURL:(NSURL *)url options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletedWithFinishedBlock)completedBlock {
    ...
    operation.cacheOperation = [self.imageCache queryDiskCacheForKey:key done:^(UIImage *image, SDImageCacheType cacheType) {
        ...
        if ((!image || options & SDWebImageRefreshCached) && (![self.delegate respondsToSelector:@selector(imageManager:shouldDownloadImageForURL:)] || [self.delegate imageManager:self shouldDownloadImageForURL:url])) {
            ...
            if (options & SDWebImageUseNSURLCache) downloaderOptions |= SDWebImageDownloaderUseNSURLCache;
            if (options & SDWebImageContinueInBackground) downloaderOptions |= SDWebImageDownloaderContinueInBackground;
            if (options & SDWebImageHandleCookies) downloaderOptions |= SDWebImageDownloaderHandleCookies;
            if (options & SDWebImageAllowInvalidSSLCertificates) downloaderOptions |= SDWebImageDownloaderAllowInvalidSSLCertificates;
            
            if (options & SDWebImageUseNSURLCache) {
                NSCachedURLResponse *cachedResponse = [self.imageDownloader cachedResponseForUrl:url options:downloaderOptions];
                image = [UIImage sd_imageWithData:[cachedResponse data]];
                image = SDScaledImageForKey([url absoluteString], image);
                
                if (!image.images) // Do not force decode animated GIFs
                {
                ...
                if (!CGSizeEqualToSize(image.size, CGSizeZero)) {
                    dispatch_main_sync_safe(^{
                        completedBlock(image, nil, SDImageCacheTypeDisk, YES);
                    });
                }
            }

            if (image && downloaderOptions & SDWebImageDownloaderUseNSURLCache) {
